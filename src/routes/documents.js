...
router.get('/:order',isLoggedIn, async (req, res) => {
   ...
   const Documents = await pool.query('SELECT idDocument FROM AustenRiggs.documents WHERE id_user = ?' ,[req.user.id]);

   res.render('documents/all', {Documents:Documents});
});

router.get('watchdoc/:order',isLoggedIn, async (req, res) => {

    const {order} = req.params;
    
    const Interviews = await pool.query('SELECT idDialogInterview FROM AustenRiggs.DialogInterviews WHERE idDocument = ?' ,[req.user.id]);
